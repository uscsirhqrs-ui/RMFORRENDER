...
/**
 * Generates HTML template for Form Submission Reminder
 */
export const getFormReminderEmailTemplate = (template, senderName) => {
  const baseUrl = getBaseUrl();
  return `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 8px; border-top: 4px solid #f59e0b;">
        <h2 style="color: #b45309;">Reminder: Form Submission Pending</h2>
        <p>Dear User,</p>
        <p>This is a gentle reminder to submit the form <strong>"${template.title}"</strong> shared by <strong>${senderName}</strong>.</p>
        
        <div style="background-color: #fffbeb; padding: 15px; border-radius: 8px; margin: 15px 0; border: 1px solid #fef3c7;">
          <p style="margin: 5px 0; color: #92400e;"><strong>Form:</strong> ${template.title}</p>
          <p style="margin: 5px 0; color: #92400e;"><strong>Status:</strong> Pending Submission</p>
        </div>
  
        <p>Please click the button below to view and fill the form.</p>
  
        <p style="margin-top: 25px;">
           <a href="${baseUrl}/data-collection" style="background-color: #f59e0b; color: white; padding: 12px 24px; text-decoration: none; border-radius: 5px; display: inline-block; font-weight: bold;">Go to Pending Forms</a>
        </p>
